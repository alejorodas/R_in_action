...
hurto_viernes_noche <- hurto_pereira_barrio_comuna_data_set_tbl %>%
  select(dia_formato,nombre_comuna,barrio,hurto_interval,hora_format_POSIXct) %>%
  filter(dia_formato=="Viernes", hurto_interval=="18-00") %>%
...
mapa_hurto_pereira_comuna_barrio <- merge(mapa_pereira_comuna_barrio,hurto_viernes_noche,by.x="nombre", by.y="nombre_comuna",  duplicateGeoms=TRUE)

tm_shape(mapa_hurto_pereira_comuna_barrio) +
  tm_fill("num_hurto",title = "Cantidad Hurtos",palette ="Blues",
          textNA = "No Reportado",style = "pretty", n=4) +
  tm_shape(mapa_hurto_pereira_comuna_barrio) +
  tm_borders() + 
  tm_text(text="nmbr_cm",size = 0.6) +
