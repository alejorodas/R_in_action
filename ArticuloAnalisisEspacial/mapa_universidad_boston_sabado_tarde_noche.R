...
# # 
# comunas <- universidad_boston_sabado %>% select(nombre_comuna,barrio)
...
mapa_barrios_dp@data<-left_join(mapa_barrios_dp@data,universidad_boston_sabado,by=c("COMUNA"="nombre_comuna","NOMBRE"="barrio"))

mapa_barrios_hurto <- subset(mapa_barrios_dp,num_hurto >= 1)

mapa_barrios_dp@data$num_hurto[is.na(mapa_barrios_dp$num_hurto)] <- 0
...
tm_shape(mapa_barrios_dp) +
  tm_borders() +
  tm_dots(col = "COMUNA", style="pretty",size="num_hurto") +
  tm_shape(mapa_barrios_hurto) +  
  tm_text(text="NOMBRE",size = 0.5)
