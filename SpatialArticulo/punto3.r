...
ggplot(temp, aes(x= temp$dia, y= factor(temp$intervalo))) + geom_tile(aes(fill =  count)) +
  ...
  scale_fill_gradient("Cantidad de Hurtos", low = "white", high = "steelblue") +
  theme_bw() + ggtitle("Hurtos por Dia y Hora en la Semana") + theme(plot.title = element_text(hjust = 0.5))
