...
# MARTES JORNADA NOCHE 18-00
myvars <- names(hurto.pereira.comuna.barrio) %in% c("Hora","MARCA","Departamento","Municipio","Sexo","hora.hurto", "Zona")
data.set.hurto.noche <- subset(hurto.pereira.comuna.barrio, hurto.inverval == "18-00" & Dia == "Martes", select = !myvars)
...
plot(mytable, type = "b", las=2,
     ...
     xlab = "",
     main = "Hurto de Celulares en Jornada Noche en el D??a Martes",
     col="red",
     pch=15,
     lty=1)
