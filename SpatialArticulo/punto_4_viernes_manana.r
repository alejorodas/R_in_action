...
# VIERNES JORNADA MANANA 06-12
myvars <- names(hurto.pereira.comuna.barrio) %in% c("Hora","MARCA","Departamento","Municipio","Sexo","hora.hurto", "Zona")
data.set.hurto.noche <- subset(hurto.pereira.comuna.barrio, hurto.inverval == "06-12" & Dia == "Viernes", select = !myvars)
...
plot(mytable, type = "b", las=2,
     ...
     xlab = "",
     main = "Hurto de Celulares en Jornada Ma??ana en el D??a Viernes",
     col="red",
     pch=15,
     lty=1)